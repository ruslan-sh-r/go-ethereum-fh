...
func testSideImport(t *testing.T, numCanonBlocksInSidechain, blocksBetweenCommonAncestorAndPruneblock int, mergePoint int) {
	...
	fork, _ := GenerateChain(&chainConfig, parent, genEngine, db, 2*int(TriesInMemory), func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{2})
	})
...
func TestSetCanonical(t *testing.T) {
	...
	// Generate and import the canonical chain
	canon, _ := GenerateChain(params.TestChainConfig, genesis, engine, db, int(2*TriesInMemory), func(i int, gen *BlockGen) {
	...
	side, _ := GenerateChain(params.TestChainConfig, genesis, engine, db, int(2*TriesInMemory), func(i int, gen *BlockGen) {
